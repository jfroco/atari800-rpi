...
{
#ifndef HAVE_LIBZ
	Log_println("This executable cannot decompress ZLIB files");
	return FALSE;
#else
	...
	int result;
	if (gzf == NULL) {
		Log_println("ZLIB could not open file %s", infilename);
		return FALSE;
	}
...
static int dcm_pass(FILE *infp, ATR_Info *pai)
{
	...
	for (;;) {
		...
		if (sector_no < pai->current_sector) {
			Log_println("Error: current sector is %d, next sector group at %d", pai->current_sector, sector_no);
			return FALSE;
		}
		...
		for (;;) {
			...
			case 0x47:
				...
				break;
			default:
				Log_println("Unrecognized sector coding type 0x%02X", sector_type);
				return FALSE;
			}
...
int CompFile_DCMtoATR(FILE *infp, FILE *outfp)
{
	...
	if (archive_type != 0xf9 && archive_type != 0xfa) {
		Log_println("This is not a DCM image");
		return FALSE;
	}
	archive_flags = fgetc(infp);
	if ((archive_flags & 0x1f) != 1) {
		Log_println("Expected pass one first");
		if (archive_type == 0xf9)
			Log_println("It seems that DCMs of a multi-file archive have been combined in wrong order");
		return FALSE;
	}
	ai.fp = outfp;
	ai.current_sector = 1;
	...
	case 2:
		...
		break;
	default:
		Log_println("Unrecognized density");
		return FALSE;
	}
	...
	for (;;) {
		...
		if (block_type != archive_type) {
			if (block_type == EOF && archive_type == 0xf9) {
				Log_println("Multi-part archive error.");
				Log_println("To process these files, you must first combine the files into a single file.");
#if defined(HAVE_WINDOWS_H) || defined(DJGPP)
				Log_println("COPY /B file1.dcm+file2.dcm+file3.dcm newfile.dcm from the DOS prompt");
#elif defined(__linux__) || defined(__unix__)
				Log_println("cat file1.dcm file2.dcm file3.dcm >newfile.dcm from the shell");
#endif
			}
			return FALSE;
		}
		...
		if ((pass_flags ^ archive_flags) & 0x60) {
			Log_println("Density changed inside DCM archive?");
			return FALSE;
		}
